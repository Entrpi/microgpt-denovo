...
class Value:                                                      # This is the scalar autograd engine that makes the whole file learn by hand.
    __slots__ = ("data", "grad", "_prev", "_backward")            # Keep each node lean: the value, its gradient, its parents, and its backward rule.

    def __init__(self, data, _children=()):
        self.data, self.grad = float(data), 0.0                   # Each scalar stores its value, its accumulated gradient, and its parents.
        self._prev, self._backward = set(_children), lambda: None # Parents define the graph; the backward function will later route blame to them.

    def _coerce(self, other): return other if isinstance(other, Value) else Value(other)    # Python numbers are allowed to enter the graph by being wrapped on demand.

    def _binary(self, other, forward, grads):                     # Binary operators create a new node and remember how to send gradients back.
        other = self._coerce(other)
        out = Value(forward(self.data, other.data), (self, other))
        out._backward = lambda: (self.__setattr__("grad", self.grad + grads[0](self.data, other.data, out.grad)), other.__setattr__("grad", other.grad + grads[1](self.data, other.data, out.grad)))
        return out

    def _unary(self, forward, grad):                              # Unary operators do the same, but with only one parent.
        out = Value(forward(self.data), (self,)); out._backward = lambda: self.__setattr__("grad", self.grad + grad(self.data, out.grad)); return out

    def __add__(self, other): return self._binary(other, lambda a, b: a + b, (lambda a, b, g: g, lambda a, b, g: g))
    __radd__ = __add__                                            # Addition just passes the upstream gradient through unchanged to both inputs.
    def __neg__(self):                                            # Negation flips the sign of the value and of the gradient.
        out = Value(-self.data, (self,))
        out._backward = lambda: self.__setattr__("grad", self.grad - out.grad)
        return out
    def __sub__(self, other): return self + (-other)              # Subtraction is addition of a negated term.
    def __rsub__(self, other): return other + (-self)
    def __mul__(self, other): return self._binary(other, lambda a, b: a * b, (lambda a, b, g: b * g, lambda a, b, g: a * g))
    __rmul__ = __mul__                                            # Multiplication uses the product rule.
    def __truediv__(self, other): return self._binary(other, lambda a, b: a / b, (lambda a, b, g: g / b, lambda a, b, g: -a * g / (b * b)))
    def __rtruediv__(self, other): return self._coerce(other) / self           # Division is multiplication by a reciprocal, with the quotient rule baked in.
    def exp(self): return self._unary(math.exp, lambda a, g: math.exp(a) * g)  # Exponentials turn logits into positive mass and are the core of softmax.
    def log(self): return self._unary(math.log, lambda a, g: g / a)            # Logs turn probabilities into additive surprise for cross-entropy.
    def sqrt(self): return self._unary(math.sqrt, lambda a, g: 0.5 * g / math.sqrt(a))  # Square roots give RMSNorm its notion of scale.
    def relu(self): return self._unary(lambda a: a if a > 0.0 else 0.0, lambda a, g: g if a > 0.0 else 0.0)  # ReLU is the tiny feed-forward nonlinearity used in the MLP.

    def backward(self):                                            # Backward walks the computation graph in reverse topological order.
        topo, seen = [], set()                                     # Build a reverse topological ordering so every node sees complete downstream gradients.
        def build(v):
            if v not in seen:
                seen.add(v)
                for child in v._prev: build(child)
                topo.append(v)
        build(self); self.grad = 1.0                               # Seed the root loss with unit gradient, then walk backward through the graph once.
...
def zeros_like(tree):                                             # Adam keeps its own tree-shaped moment buffers.
    # Adam needs moment buffers with the same nested shape as the parameters.
...
# Cross-entropy rewards probability mass on the true next character.
def loss(tokens, targets, p):
    # Training asks for high probability on the next correct character at each step.
    logits = forward(tokens, p)
