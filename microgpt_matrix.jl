...
function zeros_like(p::Params)                                      # Adam moment buffers match the parameter shapes exactly.
    vals = ntuple(i -> zeros(Float32, size(getfield(p, PARAM_NAMES[i]))...), length(PARAM_NAMES))
...
function attn_fwd(X, p)                                           # Self-attention lets each position read from its causal past.
...
function attn_bwd(dY, cache, p)                                   # Trace the learning signal backward through attention.
    ...
    dXq, dWq = dlinear(dQcat, X, p.Wq)                            # Learn how the query projection should change.
    dXk, dWk = dlinear(dKcat, X, p.Wk)                            # Learn how the key projection should change.
    dXv, dWv = dlinear(dVcat, X, p.Wv)                            # Learn how the value projection should change.
...
function block_fwd(X, p)                                          # A block is attention plus an MLP, both wrapped in residual paths.
    X1, c1 = rms_fwd(X, p.g1)                                     # Pre-norm keeps the residual stream stable.
    A, ca = attn_fwd(X1, p)                                       # Read from the causal past.
    H1 = X .+ A                                                   # Add the attention update back into the stream.
    X2, c2 = rms_fwd(H1, p.g2)                                    # Normalize again before the feed-forward branch.
    Z = linear(X2, p.W1)                                          # Expand the feature space.
    M = max.(0.0f0, Z)                                            # ReLU is the requested MLP nonlinearity.
    H2 = H1 .+ linear(M, p.W2)                                    # Project back down and add another residual update.
    return H2, (M, X2, c1, c2, ca, Z)                             # Save exactly what the backward pass will need.
end                                                               # With one block, this is the whole nonlinear body of the model.

function block_bwd(dH2, cache, p)                                 # Reverse the block in the opposite order from the forward pass.
    M, X2, c1, c2, ca, Z = cache                                  # Recover the saved block-local state.
    dM, dW2 = dlinear(dH2, M, p.W2)                               # Differentiate the second MLP projection.
    dZ = dM .* (Z .> 0.0f0)                                       # ReLU passes gradient only where its input was positive.
    dX2, dW1 = dlinear(dZ, X2, p.W1)                              # Differentiate the first MLP projection.
    ...
    dX1, datt = attn_bwd(dH1, ca, p)                              # Backpropagate through attention.
    dX_norm, dg1 = rms_bwd(dX1, c1)                               # Then through the first RMSNorm.
    dX = dH1 .+ dX_norm                                           # Add the first residual shortcut from H1 = X + ...
    grads = (g1=dg1, g2=dg2, Wq=datt.Wq, Wk=datt.Wk, Wv=datt.Wv, Wo=datt.Wo, W1=dW1, W2=dW2)
...
function forward(tok, p)                                          # Inference maps a token context to logits over the next character.
    ...
    X0 = Xtok .+ Xpos                                             # Add content and position information together.
    H, _ = block_fwd(X0, p)                                       # Run the single Transformer block.
...
function loss_and_grad(tok, nxt, p)                               # Training asks how wrong the model is and how every weight should move.
    ...
    H, cb = block_fwd(X0, p)                                      # Run the block and save its cache.
    ...
    dX0, db = block_bwd(dH, cb, p)                                # Push it through the whole Transformer block.
...
function adam!(p, g, m, v, step)                                  # Adam turns raw gradients into stable adaptive updates.
    for name in PARAM_NAMES                                        # Every trainable tensor follows the same recipe.
        pk = getfield(p, name)
        gk = getfield(g, name)
        mk = getfield(m, name)
        vk = getfield(v, name)
